...
export const isError = <T>(result: T | Error): result is Error =>
  result instanceof Error

export const satisfy =
  (matchFn: (char: SingleChar) => boolean): Parser<SingleChar> =>
  input =>
    input.length > 0 && matchFn(input[0])
...
export const or5 = <A, B, C, D, E>(
  ...
  or(parserA, or4(parserB, parserC, parserD, parserE))
...
export const and3 = <A, B, C>(
  ...
  map(and(parserA, and2(parserB, parserC)), ([resultA, otherResults]) => [
    resultA,
    ...otherResults,
  ])
...
export const and4 = <A, B, C, D>(
  ...
  map(
    and(parserA, and3(parserB, parserC, parserD)),
    ([resultA, otherResults]) => [resultA, ...otherResults],
  )
...
export const and5 = <A, B, C, D, E>(
  ...
  map(
    and(parserA, and4(parserB, parserC, parserD, parserE)),
    ([resultA, otherResults]) => [resultA, ...otherResults],
  )
...
export const all =
  ...
  input => {
    ...
    if (parsers.length === 0)
      return [error('(all) no parsers specified'), input]
...
export const none =
  ...
  input => {
    ...
    for (const parser of parsers) {
      ...
      if (!isError(result))
        return [
          error(`(none) One of the ${parsers.length} parsers satisfied`),
          input,
        ]
    }
...
export const manyN =
  <A>(
    parser: Parser<A>,
    { min = 0, max = Infinity }: ManyNLimitsType = {},
  ): Parser<A[]> =>
  input => {
...
export const optional = <A>(parser: Parser<A>): Parser<A | EmptyString> =>
  or(parser, empty)
...
export const precededBy = <A>(
  parserBefore: Parser<unknown>,
  parser: Parser<A>,
): Parser<A> => map(and(parserBefore, parser), ([_, result]) => result)

export const succeededBy = <A>(
  parser: Parser<A>,
  parserAfter: Parser<unknown>,
): Parser<A> => map(and(parser, parserAfter), ([result, _]) => result)
...
export const joinedBy = <A>(
  ...
  map(
    and(succeededBy(parser, parserInTheMiddle), parser),
    ([result1, result2]) => [result1, result2],
  )

export const char = (singleChar: SingleChar): Parser<SingleChar> =>
...
export const allButChar = (singleChar: SingleChar): Parser<SingleChar> =>
  not1(char(singleChar))

export const charSet = (set: string): Parser<SingleChar> =>
  satisfy(c => set.includes(c))
export const allButCharSet = (set: string): Parser<SingleChar> =>
  not1(charSet(set))

export const charSequence =
  (seq: string): Parser<string> =>
  input =>
    input.startsWith(seq)
      ? [seq, input.slice(seq.length)]
      : [error('no match'), input]
...
export const numberRange =
  ...
  input => {
    ...
    if (!(result >= from && result <= to))
      return [
        error(`Number must be ≥ ${from} and ≤ ${to}, but was ${result}`),
        input,
      ]
...
export const spaced = <A>(parser: Parser<A>): Parser<A> =>
  surroundedBy(many(char(SPACE)), parser)
...
export const wordChar = map(or3(letter, digit, underscore), res =>
  res.toString(),
)
...
export const natural = map(precededBy(optional(plus), digits), digs =>
  digs.reduce(
    (acc, dig, i) => acc + dig * BASE_10 ** (digs.length - (i + 1)),
    0,
  ),
)
...
export const float = map(
  ...
  ([int, nat]) =>
    ...
    (nat === 0
      ? 0
      : (Math.sign(int) * nat) / BASE_10 ** Math.trunc(Math.log10(nat) + 1)),
)
...
export const differentFrom = spaced(
  orN(
    [DIFFERENT_FROM, DIFFERENT_FROM_ALT1, DIFFERENT_FROM_ALT2].map(
      charSequence,
    ),
  ),
)
...
export const greaterThanOrEqualTo = spaced(
  orN(
    [GREATER_THAN_OR_EQUAL_TO, GREATER_THAN_OR_EQUAL_TO_ALT].map(charSequence),
  ),
)
...
export const ref = memoize(
  concat(
    map(and(letters, naturalGreaterThanZero), ([col, row]) => [
      ...col,
      row.toString(),
    ]),
  ),
...
export const additiveTerm = memoize((input => {
  const [result, rest] = or(
    mapToBinaryOperation(
      and3(
        multiplicativeTerm,
        or(add, subtract) as Parser<OperatorType>,
        additiveTerm,
      ),
    ),
    multiplicativeTerm,
  ...
  if (
    ...
    result.operator = ADD
    result.right.left = createBinaryOperation(
      NUMBER_MINUS_1,
      MULTIPLY,
      result.right.left,
    )
...
export const multiplicativeTerm = memoize((input => {
  const [result, rest] = or(
    mapToBinaryOperation(
      and3(
        exponentialTerm,
        or(multiply, divide) as Parser<OperatorType>,
        multiplicativeTerm,
      ),
    ),
    exponentialTerm,
...
export const optionallySigned = <A extends ExpressionType>(parser: Parser<A>) =>
  memoize(
    map(and(optional(sign), parser), ([signChar, result]) =>
      signChar === MINUS_SIGN
        ? createBinaryOperation(NUMBER_MINUS_1, MULTIPLY, result)
        : result,
    ),
  )
...
export const binaryToDecimal = memoize((binary: string): number =>
  binary
    ...
    .reduce(
      (acc, bit, i) =>
        acc + +(bit as BitType) * BASE_2 ** (binary.length - 1 - i),
      0,
    ),
)
...
export const hexToDecimal = memoize((hex: string): number =>
  hex
    ...
    .reduce(
      (acc, hexDigit, i) =>
        acc +
        hexDigitToDecimal(hexDigit as HexDigitType) *
          BASE_16 ** (hex.length - 1 - i),
      0,
    ),
)
...
export const operand: Parser<ExpressionType> = memoize(
  or(
    map(
      or3(
        map(binaryNumber, binaryToDecimal),
        map(hexNumber, hexToDecimal),
        numeric,
      ),
      value => ({
        type: 'numeric',
